...
namespace AutismEducationPlatform.Web.Controllers
{
    ...
    public class ParentController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SaveAnimalProgress([FromBody] AnimalProgressInputModel model)
        {
            Console.WriteLine(">>> SaveAnimalProgress tetiklendi");
            Console.WriteLine($"Model: AnimalId={model?.AnimalId}, AnimalName={model?.AnimalName}, Progress={model?.Progress}");
            if (!User.Identity.IsAuthenticated)
                return Unauthorized();

            var userId = _userManager.GetUserId(User);
            if (string.IsNullOrEmpty(userId) || model == null || string.IsNullOrEmpty(model.AnimalName))
            ...
            if (existing == null)
            {
                var newProgress = new AnimalProgress
                {
                    UserId = userId,
                    ...
                    CompletedAnimalCount = model.Progress >= 100 ? 1 : 0,
                    LastInteraction = DateTime.UtcNow,
                    InteractionCount = 1
                };
                _context.AnimalProgress.Add(newProgress);
            }
            else
            {
        ...
        public IActionResult Exam(int id)
        {
            ...
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            var examResult = _context.ExamResults.FirstOrDefault(r => r.UserId == userId && r.ExamId == id);
            if (examResult != null)
            {
                return RedirectToAction("Exams");
            }

            var questions = GetExamQuestionsById(id);
        ...
        private List<ExamQuestionViewModel> GetExamQuestionsById(int id)
        {
            // Her sınav için 5 örnek soru. Gerçek sorularla değiştirilebilir.
            var allQuestions = new Dictionary<int, List<ExamQuestionViewModel>>
            {
                { 1, new List<ExamQuestionViewModel> {
                    new ExamQuestionViewModel { Content = "Kedi hangi hayvandır?", Options = new[]{"Memeli", "Balık", "Kuş", "Böcek"}, CorrectAnswer = 0 },
                    new ExamQuestionViewModel { Content = "Köpek ne yapar?", Options = new[]{"Uçar", "Yüzer", "Havlar", "Tırmanır"}, CorrectAnswer = 2 },
                    new ExamQuestionViewModel { Content = "İnek ne üretir?", Options = new[]{"Süt", "Yumurta", "Bal", "Yün"}, CorrectAnswer = 0 },
                    new ExamQuestionViewModel { Content = "Kuş ne yapar?", Options = new[]{"Yüzer", "Uçar", "Sürünür", "Zıplar"}, CorrectAnswer = 1 },
                    new ExamQuestionViewModel { Content = "At hangi hayvandır?", Options = new[]{"Evcil", "Vahşi", "Deniz", "Küçük"}, CorrectAnswer = 0 },
                } },
                { 2, new List<ExamQuestionViewModel> {
                    new ExamQuestionViewModel { Content = "Kırmızı hangi renktir?", Options = new[]{"Soğuk", "Sıcak", "Nötr", "Pastel"}, CorrectAnswer = 1 },
                    new ExamQuestionViewModel { Content = "Mavi neyi simgeler?", Options = new[]{"Ateş", "Su", "Toprak", "Hava"}, CorrectAnswer = 1 },
                    new ExamQuestionViewModel { Content = "Sarı hangi nesnede bulunur?", Options = new[]{"Güneş", "Ay", "Ağaç", "Deniz"}, CorrectAnswer = 0 },
                    new ExamQuestionViewModel { Content = "Yeşil neyi simgeler?", Options = new[]{"Doğa", "Aşk", "Korku", "Enerji"}, CorrectAnswer = 0 },
                    new ExamQuestionViewModel { Content = "Mor hangi duyguyu simgeler?", Options = new[]{"Asalet", "Korku", "Sevgi", "Neşe"}, CorrectAnswer = 0 },
                } },
                // ... Diğer sınavlar için de benzer şekilde 5'er soru eklenebilir ...
            };
            if (allQuestions.ContainsKey(id))
                return allQuestions[id];
            // Varsayılan olarak rastgele 5 soru döndür
            return new List<ExamQuestionViewModel> {
                new ExamQuestionViewModel { Content = "Örnek Soru 1", Options = new[]{"A", "B", "C", "D"}, CorrectAnswer = 0 },
                new ExamQuestionViewModel { Content = "Örnek Soru 2", Options = new[]{"A", "B", "C", "D"}, CorrectAnswer = 1 },
                new ExamQuestionViewModel { Content = "Örnek Soru 3", Options = new[]{"A", "B", "C", "D"}, CorrectAnswer = 2 },
                new ExamQuestionViewModel { Content = "Örnek Soru 4", Options = new[]{"A", "B", "C", "D"}, CorrectAnswer = 3 },
                new ExamQuestionViewModel { Content = "Örnek Soru 5", Options = new[]{"A", "B", "C", "D"}, CorrectAnswer = 0 },
            };
        }
        ...
        {
            var userId = _userManager.GetUserId(User);
            var examList = new List<(int Id, string Title)>()
            {
                (1, "Hayvanlar Sınavı"), (2, "Renkler Sınavı"), (3, "Sayılar Sınavı"), (4, "Şekiller Sınavı"), (5, "Trafik Levhaları Sınavı"),
                (6, "Kendini İfade Etme Sınavı"), (7, "Duygular Sınavı"), (8, "Sosyal Beceriler Sınavı"), (9, "Günlük Yaşam Sınavı"), (10, "Duyusal Etkinlikler Sınavı"),
                (11, "Beyin Oyunları Sınavı"), (12, "Masallar Sınavı"), (13, "Duygusal Farkındalık Sınavı"), (14, "Problem Çözme Sınavı"), (15, "Öz Bakım Sınavı"),
                (16, "Akademik Hazırlık Sınavı"), (17, "Arkadaşlık ve Paylaşım Sınavı"), (18, "Dikkat ve Konsantrasyon Sınavı"), (19, "Motor Beceriler Sınavı"), (20, "Dil Gelişimi Sınavı")
            };
            var userResults = await _context.ExamResults.Where(x => x.UserId == userId).ToListAsync();
            var model = examList.Select(e => new ExamListViewModel
            {
                Id = e.Id,
                Title = e.Title,
                Taken = userResults.Any(r => r.ExamId == e.Id)
            }).ToList();
            return View(model);
        }
    }
